...
Rendered recipes live in the [Gallery](visualizing-data.html); per-function
arguments live in [Reference](../reference/index.html).
...
Every verb returns a `plotit` (or `plotit_composite`) object, so the native
pipe `|>` is the only composition operator. Single-plot verbs do not accept
`plotit_composite`; build panels first, then `compose_*()`.

| Family | Job | ggplot2 analogue |
|:---|:---|:---|
| `plotit()` / `encode()` | init + aesthetics | `ggplot()` / `aes()` |
...
| `style()` | theme | `theme()` |
| `export()` | file output | `ggsave()` |
| `compose_*` | multi-panel assembly | patchwork |
| `add_ggplot()` | escape hatch | arbitrary ggplot2 layer |
| `make_mark()` / `make_theme()` | user extension | — |
...
`width`/`height` are **panel** sizes (WYSIWYG baked into the plot). Without
a colour/fill mapping, `default_color` injects a single hue on both
channels; any colour/fill scale clears it automatically.
...
Legal `trans` values are filtered per aesthetic:
`identity` / `log` / `log10` / `log2` / `sqrt` / `reverse` /
`discrete` / `binned`. Colour/fill also accept `na_color`, `n_bins`,
and a diverging `mid` anchor. Date/POSIXct columns auto-route to a
date axis — do not force `trans = "identity"` on them.
...
- Package-level validation uses `cli::cli_abort` with legal values in
  the remedy. Silent argument swallowing is a defect, not a feature.
