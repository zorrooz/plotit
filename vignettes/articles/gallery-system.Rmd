---
title: "Gallery System: Pipeline Chains (v2)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Gallery System: Pipeline Chains (v2)}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

> **本页解决什么问题**：按**意图**索引的 usage 管道链——每个 API 一族最小可运行管道
> （参考 tidyplots 风格：`data |> plotit(encode(...)) |> mark_*(...)`）。只给管道链，
> 不渲染图；数据全部取自内置数据集，确保确定性（seed 固定）、单链 <5s。
> **前置**：[API System Reference](api-system.html)。
> **下一步**：→ [Transform Recipes](transform-recipes.html)。
...
| `ggplot2::mpg` | 234 行分类+连续 | 比较/关系/分面/热力 |
| `ggplot2::diamonds` | 53940 行 | 2D 分箱/六边形/密度 |
| `ggplot2::economics` | 574 行日期序列 | 趋势/面积/step |
| `ggplot2::economics_long` | 日期×变量长表 | 堆叠面积 |
...
| `ggplot2::txhousing` | 8602 行城市×月 | 趋势/分面
...
→ [Transform Recipes](transform-recipes.html)：管道前的数据整形；
→ [Use Case: Bioinformatics](use-case-bioinformatics.html) / [Publishing](use-case-publishing.html)。