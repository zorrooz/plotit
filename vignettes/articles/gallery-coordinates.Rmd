...
skills <- data.frame(
  ...
  trait = rep(c("design", "code", "research", "comms", "ops"), 2),
  score = c(7, 9, 6, 5, 8, 9, 5, 7, 8, 6)
)
skills |>
  plotit(encode(x = trait, y = score, colour = person)) |>
  mark_polygon(alpha = 0.15, linewidth = 0.8) |>
  mark_point(size = 2) |>
  project_polar() |>
  label_title("Radar = mark_polygon + project_polar")
```
...
```{r}
set.seed(5)
angles <- data.frame(a = c(rnorm(300, 0, 1), rnorm(300, 3, 0.7)))
angles |>
  plotit(encode(x = a, fill = cut(angles$a, 2, labels = c("A", "B")))) |>
