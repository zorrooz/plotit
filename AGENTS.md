...
> 当前为 0.x，API 仍在演进。以下契约面向 1.0 发布后生效。
...
约定与实现偏离时：约定描述了对的行为而实现未跟上 → 修实现；约定描述过时或不准确 → 修约定。
...
**`trans` 合法值**（统一语义——用户只需知道"如何变换"，包层内部按 aesthetic 分类调度）：

| `trans` | x/y | colour/fill/size/alpha | shape/linetype |
|---|---|---|---|
...
**`range` — 视觉输出值域**（所有 scale 含义一致："数据映射到什么视觉值上"）：
...
**x/y 的 `range`**：表示数据值域。`range=c(0,100)` 等价于 `limits=c(0,100)` + `expand=c(0,0)`。面板留白用 `project_cartesian(expand=...)`。与 `limits` 同时非 NULL 时后设置者胜，冲突时警告。
...
`text=NULL` 是安全的空操作——不修改当前标签。`text`（非 NULL）与 `reset=TRUE` 互斥，同时提供时报错。
