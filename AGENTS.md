...
| 7 | 有状态默认值必须对称清除 | 若某默认值注入了副作用（如 `default_color`→`guides(colour="none")`），则任何用户显式设置该维度的操作（`mark_*(mapping=encode(colour=...))`）都必须触发清除，不能仅依赖 `scale_*` 路径。 |
| 8 | 绝不访问 ggplot2 内部结构 | 禁止直接操作 `gg$labels`、`gg$scales$scales`、`gg$theme` 等内部槽位。所有修改必须通过 `+ labs()`、`+ guides()`、`+ theme()` 等公开 API。内部结构在 ggplot2 小版本升级时无兼容保证。 |
| 9 | 非标准求值只用 rlang | 数据掩码场景（列名查找）必须使用 `rlang::eval_tidy()`，禁止 `eval()` + `baseenv()` 组合——列名可能遮蔽 base R 函数。 |
| 10 | 警告信息必须无歧义 | "X overrides Y (latter wins)" 中的"latter"指代不明。警告必须明确说出哪个参数生效、哪个被忽略。 |
| 11 | 测试断言行为而非内部状态 | 测试应验证用户可见结果（标签内容、图例是否显示），而非 `gg$labels` 是否存在某个键或 `scales$scales[[1]]$name` 的值。内部状态会因实现路径变更而合法地改变。 |
| 12 | 契约边界必须可验证 | "±5% 容差"无法验证。契约必须用用户可见的指标定义：面板尺寸 (±1% 浮点误差)、总尺寸（衍生值，不保证）。 |
